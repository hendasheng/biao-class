...
; (function () {
    ...
    function render(state) {
        ...
        let radius = Math.floor(state.config.range / 2);
        let diameter = radius * 2;
        // 从哪开始渲染
        // 总共渲染 5 个
        // 如 1 2 3 4 5
        // 从 3 开始，左右各有两个按钮
        let start = state.currentPage - radius;
        let end = state.currentPage + radius;
        ...
        if(pageAmount <= state.config.range) {
            start = 1;
            end = pageAmount;
        }
